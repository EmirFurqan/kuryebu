...
import { MapPin, Navigation, Map, Phone, Truck, Car, Wrench, Bike, ChevronDown, ChevronUp } from 'lucide-react'
    ...
    const searchAddress = async (query, setSuggestions, setLoading) => {
        ...
        try {
            const response = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&countrycodes=tr&viewbox=27.9,41.7,30.0,40.7&bounded=1&limit=5&addressdetails=1`
            )
                ...
                <section className="bg-white/95 backdrop-blur-md p-6 md:p-8 rounded-2xl shadow-2xl max-w-lg w-full border border-gray-100">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
                            {[
                                { type: 'motor', label: 'Motor', icon: <Bike className="w-6 h-6" />, price: 120 },
                                { type: 'araba', label: 'Araba', icon: <Car className="w-6 h-6" />, price: 180 },
                                { type: 'servis', label: 'Servis', icon: <Wrench className="w-6 h-6" />, price: 250 },
                                { type: 'ticari', label: 'Ticari', icon: <Truck className="w-6 h-6" />, price: 320 }
                            ].map(opt => (
                                <button
                                    key={opt.type}
                                    type="button"
                                    onClick={() => {
                                        ...
                                        setEstimatedPrice(opt.price)
                                    }}
                                    className={`flex flex-col items-center justify-center p-2 rounded-lg border 
          ${selectedVehicle === opt.type ? 'bg-[#ED6B1D] text-white border-[#ED6B1D]' : 'bg-white text-gray-700 border-gray-300'}
          hover:bg-[#ED6B1D]/80 hover:text-white transition-colors duration-150
          w-full h-full min-h-[50px]`}
                                >
                                    {opt.icon}
                                    <span className="text-sm font-medium mt-2">{opt.label}</span>
                                </button>
                            ))}
                        </div>
                    ...
                    dangerouslySetInnerHTML={{
                        __html: JSON.stringify({
                            ...
                            'areaServed': locations.map(loc => ({
                                ...
                                'containsPlace': loc.neighborhoods.map(n => ({
                                    '@type': 'Place',
                                    'name': getNeighborhoodName(n)
                                }))
                            })),
                            'potentialAction': {
                                '@type': 'OrderAction',
                                'target': {
                                    '@type': 'EntryPoint',
                                    'urlTemplate': 'https://wa.me/905518930451',
                                    'inLanguage': 'tr-TR',
                                    'actionPlatform': [
